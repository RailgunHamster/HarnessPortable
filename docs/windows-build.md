...
- **管理界面是独立窗口**（主窗口工具栏「管理 / 设置」，或托盘右键「管理 / 设置」）：隧道/直连维护、应用设置、检查更新都在这里，不占工作区标签；主窗口关到托盘、或工作区本身出问题时依然打得开
- **管理窗口配色跟随系统深浅色**（Windows 设置 → 个性化 → 颜色 → 选择模式）：token 直接取自 DSH 前端的设计系统（`dsh-client-ui-theme`），深浅两套整本字典热切换，`%APPDATA%\HarnessPortable\flicker.log` 里的 `[theme]` 行会记下当前用的是哪套。主窗口与其它弹窗仍是浅色样式
- 托盘右键「检测并更新」：不打开主窗口也能检查并安装新版本，结果用气泡提示
